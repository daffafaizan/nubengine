...
class VBEPostings:
    ...
    def vb_encode_number(number):
        ...
        encoded_number = []
        while number > 0:
            encoded_number.append(number % 128)
            if number < 128:
                break
            number = number // 128
        encoded_number[0] += 128
    ...
    def vb_encode(list_of_numbers):
        ...
        for n in list_of_numbers:
            encoded_numbers.extend(VBEPostings.vb_encode_number(n))
    ...
    def encode(postings_list):
        ...
        for i in range(len(postings_list) - 1):
            diff = postings_list[i + 1] - postings_list[i]
    ...
    def decode(encoded_postings_list):
        ...
        for i in range(1, len(decoded_gap_list)):
            decoded_gap_list[i] += decoded_gap_list[i-1]
            
        return decoded_gap_list
