...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        encoded_tf_list = self.index_file.read(tf_list_length)

        return (self.postings_encoding.decode(encoded_postings_list), self.postings_encoding.decode_tf(encoded_tf_list))
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        encoded_postings_length = len(encoded_postings_list)
        encoded_tf_list_length = len(encoded_tf_list)

        for idx, doc_id in enumerate(postings_list):
            if doc_id not in self.doc_length:
                self.doc_length[doc_id] = 0
            self.doc_length[doc_id] += tf_list[idx]
        ...
        self.postings_dict[term] = (start_position, postings_length, encoded_postings_length, encoded_tf_list_length)
